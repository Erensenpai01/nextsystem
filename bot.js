...
client.on('message', message => {
  ...
  if (message.content.startsWith(prefix + "move")) {
   if(!message.member.roles.find(role => role.name === "Move")) return message.reply("__**``Move`` ليس لديك رول**__"); 
...
client.on('message', message => {
  ...
  if (message.content.startsWith(prefix + "goto")) {
   ...
   var chan2 = mention.voiceChannel;
     ...
     var permissions = chan2.permissionsFor(message.member.user);
     if (!permissions.has('CONNECT') || !permissions.has('VIEW_CHANNEL')) {
      return message.reply('__**ليس لديك الصلاحية لدخول إلى القناة الموجود فيها  هذا العضو **__');
    }
    console.log('hi')
    let log = message.guild.channels.find( channel => channel.name === "move-log");
...
var args2 = args.join(' ');
	...
	var channel = msg.guild.channels.find(chann => chann.id === "620781324034375680");
...
client.on("message", message => {
    if (message.content === (prefix + "help")) {
     const embed = new Discord.RichEmbed() 
         ...
         ${prefix}link : ة الدعوة إلى السيرفر
         ------------------------------
         ${prefix}ban  : حظر العضو من السيرفر
         ${prefix}kick : طرد العضو من السيرفر
         ${prefix}mute : إعطاء ميوت للعضو
         ${prefix}unmute : فك الميوت  عن العضو
         ${prefix}report : الإبلاغ عن عضو
...
client.on("guildMemberAdd", memb => {
  ...
  const channel = memb.guild.channels.find('id', '640676342450356224'); //ايدي الروم
if (!channel) return;
channel.send(`**<@${memb.user.id}> Welcome To ``IDK,``  ** ❤️ `)  
